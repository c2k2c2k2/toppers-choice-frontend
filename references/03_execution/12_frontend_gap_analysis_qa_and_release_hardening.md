...
- Admin record search now uses one shared debounced input pattern across the API-backed admin workspaces, and the affected TanStack queries keep previous list data visible during refetch so slow typing no longer causes repeated full-screen loading flicker.

## Verification Notes
- Verified frontend with `pnpm lint` and `pnpm build` after the contract regeneration and hardening changes.
- Re-verified frontend with `pnpm lint` and `pnpm build` after hardening admin record search behavior across notes, content, assessments, plans, users, notifications, ops, and CMS lists.
