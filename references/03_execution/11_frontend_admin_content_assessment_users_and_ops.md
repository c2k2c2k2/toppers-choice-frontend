...
- The dedicated question-entry screen now keeps operator-selected settings in a focused Zustand store, auto-suggests the next question code from the selected subject, and adds a `Publish & next question` flow that resets content while preserving the repeated admin settings needed for high-volume entry.
- The question editor layout is now denser and more production-like for data entry: the route tabs and tall status cards were removed, preview is toggled on demand, the action bar is pinned to the bottom, and the Tiptap toolbar was reshaped into a more traditional editor strip instead of stacked pill controls.
- Question entry is now Unicode-first for Marathi authoring in admin, while the shared renderer still tolerates older encoded question content.
- Admin publish UX now exposes a checklist before publish so missing question code, incomplete bilingual statement content, incomplete option content, or missing answers are visible before the request reaches the backend.
...
- Re-verified with `pnpm lint` and `pnpm build` after compacting the `/admin/questions` table rows to show only useful authored question context and wiring guarded delete actions into the admin question bank.
- Re-verified with `pnpm lint` and `pnpm build` after the question-entry UX refresh for persisted settings, subject-based code suggestions, sticky publish actions, Unicode-first Marathi authoring, and the publish readiness checklist.
